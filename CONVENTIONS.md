...
- **Shell-script tests** for anything under `bin/` (host bootstrap, Gitea bootstrap, future operator scripts) live under `bin/tests/test-*.sh` and are gated behind `RUN_SHELL_TESTS=1`. They need Docker + 60–120s to run, so the default vitest matrix skips them; the dedicated `shell-tests` CI job runs them. The first invocation of each test must assert the script's first-run effect; a second invocation must assert idempotency (every step reports `skipped` / `exists` / `configured`, never `created` / `installed`).
