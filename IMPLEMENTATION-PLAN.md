...
**Phase-a: appendix #4 wave 1 of 3 complete.** 32 / 32 base PRs merged + 5 post-32 + 1 scoping (#41) + 3 wave-1 (#42, #43, #44) landed. `main` is at commit `263a424` (PR-A wave-1 close). Wave 1 shipped: real source-binding status probe (PR-A / #42), Badge legibility + StatusPill (PR-E / #43), source-asana v2 handshake + event_type + Light summary helper (PR-F / #44). Wave 2 (PR-B observability core, PR-G AsanaClient + snapshot) dispatches next; wave 3 (PR-C Review tab, PR-D Reports tab, PR-H asana-project compiler, PR-I source-webhook, PR-J output-webhook) lands after wave 2's SSE infrastructure ships. Full PR detail in `docs/plan-appendix/phase-a-4-observability.md`.
...
- [ ] **Appendix #4 PR-G merged** — `AsanaClient` + snapshot enrichment (`snapshotMode: 'on-event'|'periodic'|'off'`); emits second SourceEvent with `content_kind: 'asana-project'`.
- [ ] **Appendix #4 PR-H merged** — `asana-project` Compiler template (Polish merge prompt mirrors PoC; registers `'asana-project'` in `CONTENT_KINDS`).
- [ ] **Appendix #4 PR-I merged** — generic `source-webhook` adapter (HMAC + replay-stable event_id + `contentKindMap` jsonpath; `reviewMode: 'review'` default).
- [ ] **Appendix #4 PR-J merged** — generic `output-webhook` adapter (signed outbound + retry/jitter + `output_deliveries` audit table).
