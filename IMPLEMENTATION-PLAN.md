...
**Phase-a: `0.1.0-a` shipped 2026-05-01; appendix #5 (production scheduling) + appendix #6 (pilot autonomy) both complete.** Core 32 / 32 PRs + 5 post-32 + 1 appendix-#4 scoping (#41) + 10 appendix-#4 PRs (#42–#51) + 3 appendix-#5 PRs (#53, #54, #55) + 3 appendix-#6 PRs (N1 `de02fd7` / #56, N3 `aa64e10` / #57, N2 `5790bb9` / #58). After appendix #6: webhook receiver mounts in production via `pnpm opencoo`; webhook-native adapters (source-webhook, source-asana) produce `ingestion_intake` rows directly via the receiver's `enrichEvents` fast path; `HttpMcpToolClient` + populated `AgentRunnerRegistry` enable Heartbeat / Lint to fire on cron when `MCP_BEARER_TOKEN` is set (Surfacer omitted until v0.2 wires the template catalog). Full appendix-#4 PR detail in `docs/plan-appendix/phase-a-4-observability.md`; appendix-#5 (M1/M2/M3) summarised in §1.2.13; appendix-#6 (N1/N2/N3) summarised in §1.2.14 below.
