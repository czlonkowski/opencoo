...
**Phase-a: appendix #4 wave 2 of 3 complete.** 32 / 32 base PRs merged + 5 post-32 + 1 scoping (#41) + 3 wave-1 (#42, #43, #44) + 2 wave-2 (#45, #46) landed. `main` is at commit `c26f7cb` (PR-G wave-2 close). Wave 2 shipped: observability core — Activity 5th tab + first SSE route + agent-run lifecycle bus + production BullMQ queue handle wired in `start.ts` (PR-B / #45); AsanaClient + snapshot enrichment + Light-summary wiring closing PR-F's deferred TODO (PR-G / #46). Wave 3 (PR-C Review tab, PR-D Reports tab, PR-H asana-project compiler, PR-I source-webhook, PR-J output-webhook) dispatches next; PR-J is sequential after PR-I (verifier symmetry). Full PR detail in `docs/plan-appendix/phase-a-4-observability.md`.
