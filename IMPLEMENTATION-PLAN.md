...
**Phase-a: 27 / 32 PRs merged** (plus the §0 pre-coding gate). `main` is at commit `353426d`; full repo 1496 passed | 2 skipped. Docker installed via colima for the wiki-gitea contract suite.
...
**Residual advisories filed across PRs 7-27** (all non-blocking, v0.2 hardening or future-PR reactivity): listed in each PR's body on GitHub. Tracked for the phase-a exit-gate `CHANGES-v0.1.md` draft.

**Next PR**: PR 28 `engine-self-operating/review-dashboard` — Review Dashboard server-side endpoints + 4 item types (source-binding, Lint, Surfacer, marketplace per §7.3). Server-side Gitea-team membership recheck on every state-changing endpoint; CSRF tokens; `SameSite=Strict`; audit log row per admin action; sovereignty-diff confirmation on `llm_policy` edits (THREAT-MODEL §3.13). First PR of §1.2.7.
