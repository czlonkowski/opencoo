...
**Phase-a: `0.1.0-a` shipped 2026-05-01; appendix #5 (production scheduling) complete.** Core 32 / 32 PRs + 5 post-32 + 1 appendix-#4 scoping (#41) + 10 appendix-#4 PRs (#42–#51) + 3 appendix-#5 PRs (M1 `bc23026`, M2 `2838fdf`, M3 — this branch). Pilot runbook landed at `docs/pilot-runbook.md`; webhook → wiki dequeue is active in production from `pnpm opencoo`; AgentDispatcher infrastructure ready for runner wiring (PR 23+, phase b). Full appendix-#4 PR detail in `docs/plan-appendix/phase-a-4-observability.md`; appendix-#5 (M1/M2/M3) summarised in §1.2.13 below.
...
| appendix #5 PR-M3 (this branch) | Pilot runbook + real-data smoke recipe | PR-M1, PR-M2 | `tests/smoke-real-data.test.ts` (17 assertions: `parseArgs` defaults / `--boot` / `--help` / `--port`; `assertEnv` against `production-composition.ts`'s required set; `pollUntil` happy / timeout / propagated-error paths; pinned timeout constants) | New `docs/pilot-runbook.md` — operator-facing runbook walking pre-flight → first boot → bind a real Asana source → real-data smoke → rollback. New `scripts/smoke-real-data.ts` — operator probe (provisions transient domain + webhook binding, posts signed fixture, polls for `webhook_events` + `ingestion_intake` rows, tears down). `pnpm smoke:real-data` registered. No engine code; no schema changes; no new env vars | `pnpm vitest run tests/smoke-real-data.test.ts` (17/17) + `pnpm lint && pnpm typecheck && pnpm test` green at root | ~5 |

Known gap surfaced by appendix #5 and tracked for phase b: `AgentRunnerRegistry` boots empty — `agents seed` writes the rows but production runners need `HttpMcpToolClient` wiring (PR 23+). The runbook §8 names this as a deliberate v0.1 limitation; `/api/admin/scheduler` enumerates the seeded rows so operators see the surface.
