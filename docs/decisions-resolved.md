...
- **OKF v0.1 conformance.** The on-disk wiki **is** a Google Open Knowledge Format v0.1 bundle (markdown + YAML frontmatter; spec at `GoogleCloudPlatform/knowledge-catalog`) — the format opencoo already used, now standardised, so conformance is a thin contract not a redesign. `@opencoo/shared/page-spec` defines it: every concept page needs a non-empty `type` (OKF's one required field); `index.md` carries no frontmatter except a bundle-root `okf_version`; `log.md` uses ISO date headings; consumption is permissive (unknown types/keys/broken links tolerated). Enforced by a `wikiWrite` gate (`off | warn | throw`, default `warn`; flips to `throw` after the pilot migration). Gives data portability — a domain repo `git clone`s as a standard OKF bundle — and interop with any OKF producer/consumer.
