...
- **PoC reference workflows live in the partner's n8n instance** as a four-workflow set: an event-listener (~13 nodes), a wiki-updater (~20 nodes), and a webhook-register / webhook-manager pair — canonical reference for what the adapter should emit. The handshake echo, `deriveEventType` function, monitored-project filter, Light-tier summary, and snapshot+merge prompt are all in those workflow JSONs (browse from `docs/local/` for the concrete IDs — gitignored, not for public commits).
...
The `0.1.0-a` tag follows once all ten PRs (A–J) are green and §1.3 phase-a exit gate clears. Pilot cutover specifically depends on F + G + H being green; I and J are valuable but not cutover-blocking if the design partner uses `source-asana` exclusively for its Asana state ingestion.
...
- **Asana-as-MCP port (Asana as MCP tool surface for agents).** Separate decision: would live as a `packages/asana-mcp-server/` analog to `gitea-wiki-mcp-server`. Useful for Heartbeat / Wiki Chat to read live Asana, but distinct from this appendix's "ingest state to wiki" goal. Defer to v0.2 unless a partner asks.
- **Asana project-discovery port.** Weekly REST-list-and-notify pattern; this is a Surfacer-class job (proposes "enable monitoring on project X?" candidate). Belongs in phase-b alongside SkillMiner — same agent shape, different content. The `source-webhook` adapter from PR-I is enough to bridge it from n8n in the meantime.
- **Asana-reader port.** Trivial REST wrapper; subsumed into PR-G's `AsanaClient` as a method. No separate package.
...
PoC reference workflows (READ-ONLY — production traffic on the partner's n8n instance, do not modify during port): the wiki-updater workflow (snapshot+merge prompt), the event-listener workflow (handshake + deriveEventType + monitored filter + Light summary), and the webhook-register / webhook-manager pair. Concrete workflow IDs live in `docs/local/` (gitignored). Copy logic, not credentials.
