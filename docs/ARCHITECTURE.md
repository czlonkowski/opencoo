...
3. **Gitea is the storage backend, the agent-facing MCP server, AND the human-review surface for compiled wikis.** `packages/gitea-wiki-mcp-server/` (already shipping, in partner production) is the canonical MCP surface. Operators read, comment on, and merge agent-authored changes through Gitea's web UI — so Gitea must be **operator-reachable in every install**, not just container-network-reachable. The OSS-recommended shape is a sub-domain (`gitea.<your-domain>`) on the same reverse proxy as opencoo; the login modal renders a clickable "Open Gitea" link driven by the optional `GITEA_PUBLIC_URL` env var (see `docs/pilot-runbook.md` §14). Do not build a custom wiki MCP server.
