...
- **Activity feed empty after a webhook delivery.** check `webhook_events` directly: `psql "$DATABASE_URL" -c "SELECT id, binding_id, signature_ok, received_at FROM webhook_events ORDER BY received_at DESC LIMIT 5;"`. `signature_ok = false` rows mean HMAC failed; the receiver returns 401 + writes the failed row + DLQ-enqueues, but does NOT emit a structured stderr log line for the rejection in v0.1. To diagnose further, enable Fastify's request logger by setting the receiver's `logger: true` flag (currently composition-time only; surface in admin UI is a v0.2 deferral). Most likely cause: the binding's `webhook_secret_credentials_id` points at a credential whose plaintext doesn't match what the upstream signs with — re-issue the webhook on the upstream side to trigger the handshake again.
...
- **Heartbeat / Lint / Surfacer scheduled agents fire on cron when `MCP_BEARER_TOKEN` is set.** PR-N3 (phase-a appendix #6) wires the production `HttpMcpToolClient` + `AgentRunnerRegistry`. `agents seed` writes the `agent_instances` rows with `schedule_cron` populated, the orchestrator composes the registry at boot, and the `AgentDispatcher` (PR-M2) registers the BullMQ recurring jobs. Boot-tolerance: when `MCP_BEARER_TOKEN` (or its `_FILE` variant) is unset OR `HttpMcpToolClient` construction throws, the orchestrator logs `mcp_http.unavailable` and boots with an EMPTY registry — the management UI stays alive, the webhook → wiki path still works, and the `/api/admin/scheduler` route enumerates the seeded rows but every dispatched job records a failed `agent_runs` row pointing at the registry miss. Per-dispatch domain slug resolution comes from `agent_instances.scope_domain_ids[0]` (no new env var); v0.2 will add a manual-trigger CLI.
