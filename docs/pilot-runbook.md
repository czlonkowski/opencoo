...
### 11.3 Runtime gotchas — partner cutover findings

Three operational gotchas surfaced during the 2026-05-11 partner cutover (wave-12 scoping doc §G15). None are bugs; all are operator-facing footguns worth memorising.

**Compose secrets file permissions (mode 0440 → mode 0644).** When `compose.partner.yml` mounts secrets via `secrets:` (Docker compose's native secret-file primitive), the default file mode is `0440` owned by root inside the container. The engine container runs as UID 1001 (non-root by design — THREAT-MODEL §3.1) and gets `permission denied` reading the mounted file at `/run/secrets/*`. Workaround: `chmod 0644 secrets/*.txt` on the HOST before `docker compose up`. The files contain bearer tokens, not high-value secrets, so the wider mode is acceptable — operators should rotate the tokens periodically (every 90 days is a reasonable cadence; rotate by editing the file then `docker compose up -d --force-recreate` per the next gotcha).

**`docker compose restart` does NOT reload `env_file:`.** Changing the contents of a file referenced via `env_file:` in `compose.partner.yml` and then running `docker compose restart <service>` will NOT pick up the new values — `restart` only restarts the existing container with the same env baked in at create time. To reload env vars after editing an `env_file:` source, use `docker compose up -d --force-recreate <service>`. This applies to BOTH the engine (`opencoo`) and the MCP server (`gitea-wiki-mcp-server`) services. Symptom: an operator rotates `MCP_BEARER_TOKEN`, restarts the engine, and engine logs continue to use the OLD token until the next full deploy — the restart was a no-op for env reload.

**`postgres-init.sh` permissions (mode 0755, not 0700).** The init script mounted into the postgres container at `/docker-entrypoint-initdb.d/postgres-init.sh` must be world-readable so the postgres user inside the container (NOT root) can read it. The container's init phase logs `permission denied` if the file is mode `0700` (the default `chmod +x` leaves on a freshly-cloned script) and the cluster boots empty — no `domains` table, no `users` table, no schema at all. Fix: `chmod 0755 deploy/postgres-init.sh` on the host before `docker compose up -d`. Symptom worth recognising: `engine logs show "relation \"domains\" does not exist"` at every boot, but `psql` shows the database itself is reachable and empty.
