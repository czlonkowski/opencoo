 ...
 * the engine /ready endpoint to gate traffic via the reverse proxy.
 *
 * The test seam is the pool's `query` method — `vi.fn` mocks in
 * tests, real `pg.Pool` in prod. Avoiding pglite here keeps the
 * probe path dependency-free.
 */
