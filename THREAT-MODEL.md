...
8. **Append-only tables** (`page_citations`, `redaction_events`, `erasure_log`, `miner_suppressions`, `agent_runs`) are never UPDATEd or DELETEd by engine code. Retention pruning in Cleanup is the only exception. `catalog_candidate.status` and `catalog_candidate.reviewed_*` are sanctioned UPDATE targets (status transitions `detected` → `drafted` → `reviewing` → `approved`/`rejected` → `promoted`); the table is mutation-adjacent and explicitly excluded from this invariant.
