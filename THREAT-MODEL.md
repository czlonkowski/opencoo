...
8. **Append-only tables** (`page_citations`, `redaction_events`, `erasure_log`, `miner_suppressions`, `llm_usage_debug`) are never UPDATEd or DELETEd by engine code. Retention pruning in Cleanup is the only exception. **One carve-out** (PR 19 / plan #87 Q11): `agent_runs` allows a single UPDATE per row to terminalize `status` from `running` to `success` | `failed` | `timeout`, plus the associated terminal columns (`ended_at`, `output`, `tool_calls`, `tokens_in/out`, `cost_usd`, `latency_ms`, `error_class`). The harness builds the UPDATE with a `WHERE status = 'running'` guard so a terminal row can never be re-mutated; once status is terminal, the row is append-only. `catalog_candidate.status` and `catalog_candidate.reviewed_*` are sanctioned UPDATE targets (status transitions `detected` → `drafted` → `reviewing` → `approved`/`rejected` → `promoted`); the table is mutation-adjacent and explicitly excluded from this invariant.
