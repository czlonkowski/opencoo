...
Phase-a of the v0.1 engine is implemented and deployed: `packages/` holds real TypeScript src across `shared`, `engine-ingestion`, `engine-self-operating`, `cli`, `ui`, the `adapters/*` family, and the pre-existing `gitea-wiki-mcp-server`; the toolchain (pnpm + Turborepo + Drizzle + vitest + ESLint boundary rules + Husky + CI) is in place. The design-partner pilot runs this engine in production (phase-a images tagged `0.1.0-a.N`), in parallel with the legacy n8n workflows during a pipeline-by-pipeline cutover. The repo also contains two kinds of design documents:
...
|---|---|---|
| Status | Still in pilot production; retired pipeline-by-pipeline as opencoo replaces each workflow | Phase-a implemented + deployed to the design-partner pilot (tags **`0.1.0-a.N`**); phases b/c in progress |
...
| LLM access | Direct HTTP to provider from n8n nodes | Vercel AI SDK inside the engine process — no external gateway (the deployed pilot routes all three tiers through Azure OpenAI via Entra ID) |

**Phase-a opencoo TypeScript is written and running in the design-partner pilot** alongside the legacy n8n workflows; the cutover proceeds pipeline-by-pipeline (architecture.md §17 Resolved, "Pilot migration path"). What the pilot reveals in production — edge cases, prompts needing more iteration, flows that didn't survive contact with reality — continues to feed architecture.md refinement and the remaining phase work. v0.1's parity target is the finished PoC.
...
The PoC is where every product assumption was **empirically validated** — the two-pass ingestion (Worker classify → Thinker compile), the three-tier model strategy, the Karpathy wiki pattern, the heartbeat-as-daily-task delivery, the lint-and-merge loop, and the Gitea-MCP-as-agent-interface all exist and work today. The canonical operational walkthrough lives in `docs/local/` (browse via filesystem; do not name files from that directory in public commits). It contains the **real production prompts** for the live LLM calls; they were seeded into `packages/shared/src/prompts/` (locale-prefixed TS modules, e.g. `pl-classifier.ts`, `pl-heartbeat.ts`) and have been iterated against live feedback — do not rewrite them from scratch.
...
- Phase-a is implemented across `packages/shared`, `engine-ingestion`, `engine-self-operating`, `cli`, `ui`, and `adapters/*`, alongside the pre-existing `gitea-wiki-mcp-server` — all running in the design-partner pilot. Phase b (catalog-skills + SkillMiner) and phase c (overlay + marketplace live-fetch) remain to build; check `IMPLEMENTATION-PLAN.md` §1.1 for the current wave and what is still scaffold vs. complete before assuming a subsystem exists.
...
- **Commands.** The toolchain is live (landed as `IMPLEMENTATION-PLAN.md` §0). Root `package.json` scripts: `pnpm build` (turbo), `pnpm typecheck`, `pnpm test` (vitest), `pnpm test:injection`, `pnpm test:e2e`, `pnpm test:live-pilot`, `pnpm lint` (root + i18n), `pnpm format`, and `pnpm opencoo` (CLI). ESLint enforces the five custom `@opencoo/eslint-plugin` boundary rules (`no-cross-engine-import`, `no-direct-gitea-write`, `no-direct-llm-sdk`, `no-feature-env-vars`, `no-update-append-only`) plus the `import-x/no-cycle` cycle guard from `eslint-plugin-import-x`; the `no-cross-engine-import` rule's `appliesTo` option also covers the self-op import direction. `CONVENTIONS.md` §3 documents the three test tiers and how each is invoked; `architecture.md` §14.5 is authoritative for the `opencoo` CLI verbs.
