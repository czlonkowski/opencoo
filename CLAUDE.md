...
- **Commands.** The toolchain is live (landed as `IMPLEMENTATION-PLAN.md` §0). Root `package.json` scripts: `pnpm build` (turbo), `pnpm typecheck`, `pnpm test` (vitest), `pnpm test:injection`, `pnpm test:e2e`, `pnpm test:live-pilot`, `pnpm lint` (root + i18n), `pnpm format`, and `pnpm opencoo` (CLI). ESLint enforces the custom `@opencoo/eslint-plugin` boundary rules (`no-cross-engine-import`/`-selfop`, `no-direct-gitea-write`, `no-direct-llm-sdk`, `no-feature-env-vars`, `no-update-append-only`, `no-cycle`). `CONVENTIONS.md` §3 documents the three test tiers and how each is invoked; `architecture.md` §14.5 is authoritative for the `opencoo` CLI verbs.
