...
- `@opencoo/engine-ingestion` — Fastify boot + BullMQ wiring + readiness probes (`/health`, `/ready` gates on Postgres + Redis + Gitea); intake + four-level dedupe + webhook receiver; classifier with XML spotlighting; compiler with atomic per-run `wikiWrite` + `page_citations` + `Worldview-Impact` git trailer; five scheduled pipelines (Scanner, Compilation Worker, Index Rebuilder, Review Dispatcher, Cleanup) (#15, #16, #17, #18, #19, #20).
...
| `0002_agent_runs_fk_backfill.sql`                         | `agent_definitions`, `agent_instances`, `agent_runs`, `automation_candidates`, `automation_deployments`, `marketplace_updates` + 5 enums; backfills `llm_usage.run_id` and `page_citations.compiled_by_run_id` FKs to `agent_runs` | `agent_runs.skills_used jsonb default '[]'::jsonb` carries `{slug, version, sha, source}` per Builder run (#4)                                                                                 |
